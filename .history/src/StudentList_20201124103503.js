...
state = {
    ...
    //   }
      ...
      render() {
        return (
            <div>
                {this.state.students.map(student => <li> {student.firstName} {student.lastName} <br></br> Email: {student.email} <br></br>Company: {student.company} <br></br>Skill: {student.skill} <br></br> Average: {student.grades.reduce((a, b, c, d, e, f, g) => ( a + b + c + d + e + f + g) ) / student.grades.length}</li>)}
