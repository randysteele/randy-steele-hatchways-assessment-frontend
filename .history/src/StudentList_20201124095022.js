...
state = {
    ...
    //   }
      ...
      render() {
        return (
            <div>
                {this.state.students.map(student => <li> {student.firstName} {student.lastName} <br></br> Email: {student.email} <br></br>Company: {student.company} <br></br>Skill: {student.skill} <br></br> Average: {this.state.student.grades.reduce((previous, current) => current += previous / this.state.students.grades.length)}</li>)}
